...
for i in ProduceDictionary:
    if round(float(ProduceDictionary[i]['cost'])*float(ProduceDictionary[i]['amt_sold']), 2) == float(ProduceDictionary[i]['total']):
        next
    else:
        print('\n '+i+' is wrong:')
        print('Started total: $' +
              str(ProduceDictionary[i]['total'])+'\nCalculated total: $' + str(round(ProduceDictionary[i]['cost']*ProduceDictionary[i]['amt_sold'], 2)))
