...
# medical is a list of dictionaries (those dicts being room-number, use, sq-ft, and price)
list1 = datastore["medical"]
# now iterate through each dictionary and print out the specific value for that room-number (the unqiue identifier)
for i in list1:
    outfile.write(str(i['room-number'])+','+str(i['use']) +
                  ','+str(i['sq-ft'])+','+str(i['price'])+'\n')
outfile.close()
